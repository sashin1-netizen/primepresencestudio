    ...
    >
      <section className="relative mb-14 overflow-hidden border border-[#C8A348]/20 bg-[#080706] shadow-[0_35px_100px_rgba(0,0,0,.45)]">
        <div className="relative min-h-[320px] sm:min-h-[430px] lg:min-h-[520px]">
          <Image src={`${basePath}/media/ppa-hero-collateral.webp`} alt="Prime Presence Atelier black and gold brand collateral staged on a dark marble surface" fill priority sizes="100vw" className="object-cover object-center" />
      ...
      <div className="grid gap-px overflow-hidden rounded-[var(--radius-md)] bg-white/10 md:grid-cols-2">
        {services.map((service, index) => {
          ...
          return (
            <article key={service.slug} className={`group flex flex-col bg-[#080808] p-8 transition-colors hover:bg-[#0c0b09] sm:p-10 ${completesOddRow ? "md:col-span-2" : ""}`}>
              <div className={completesOddRow ? "max-w-3xl" : undefined}>
                ...
                <Link href={service.slug === "connect" ? "/connect" : "/contact"} className="link-arrow mt-7 inline-flex self-start">{service.slug === "connect" ? "Explore Connect" : "Discuss this offer"} <span aria-hidden="true">→</span></Link>
              </div>
            </article>
          );
        })}
      ...
      <section className="mt-14 grid overflow-hidden border border-white/10 bg-[#070706] lg:grid-cols-[1.05fr_.95fr]">
        <div className="relative min-h-[300px] lg:min-h-[430px]">
          <Image src={`${basePath}/media/ppa-studio-scene.webp`} alt="Dark premium creative studio interior with warm gold lighting" fill sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover object-center" />
