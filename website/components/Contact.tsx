...
export default function Contact() {
  ...
  <form onSubmit={submit} noValidate className="grid gap-5 rounded-[2rem] border border-white/10 bg-[#0b0b0b] p-6 sm:grid-cols-2 sm:p-8">
    <label>Name *<input name="name" autoComplete="name" className={field} /></label><label>Business name<input name="business" autoComplete="organization" className={field} /></label><label>Email *<input name="email" type="email" autoComplete="email" className={field} /></label><label>Service<select name="service" className={field}>{services.map(s => <option key={s.slug}>{s.title}</option>)}</select></label><label>Approximate budget<select name="budget" className={field}><option>Not decided yet</option><option>R10,000–R25,000</option><option>R25,000–R50,000</option><option>R50,000+</option></select></label><label>Preferred timeline<select name="timeline" className={field}><option>Flexible</option><option>Within 1 month</option><option>1–3 months</option><option>3+ months</option></select></label><label className="sm:col-span-2">Project details *<textarea name="details" minLength={20} className={`${field} min-h-36`} aria-describedby="form-error" /></label>
