  ...
  return (
    <footer className="border-t border-[#C8A348]/20 bg-[#030303] px-5 py-14 sm:px-8 sm:py-16">
      <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <img src={`${basePath}/media/prime-presence-logo.webp`} alt="Prime Presence Atelier" className="h-auto w-[190px] object-contain sm:w-[240px]" />
          <p className="mt-5 max-w-md text-sm leading-7 text-gray-400">{site.tagline}</p>
          {contactDetails.email ? <a href={`mailto:${contactDetails.email}`} className="mt-6 inline-flex min-h-11 items-center text-sm text-gray-300">{contactDetails.email}</a> : <p className="mt-6 text-sm text-gray-500">Public contact details are being finalised for launch.</p>}
          ...
          <p className="mt-2 text-[10px] uppercase tracking-[.16em] text-gray-500">{site.location} · Founder-led · Remote delivery available</p>
        </div>
