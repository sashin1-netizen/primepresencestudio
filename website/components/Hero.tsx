...
export default function Hero({ location }: { location: string; media?: { src: string; alt: string; decorative: boolean } }) {
  ...
  return (
    <section id="top" className="relative min-h-[100svh] overflow-hidden bg-[#030302] text-white">
      ...
      <div className="container-site relative z-10 grid min-h-[100svh] items-center gap-10 pb-14 pt-28 lg:grid-cols-[.78fr_1.22fr] lg:gap-6 lg:pb-8 lg:pt-24">
        <motion.div
          initial={false}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: reduced ? 0 : 0.8, ease: [0.22, 1, 0.36, 1] }}
          className="relative max-w-[650px] lg:pl-12"
        >
        ...
        <motion.div
          ...
          transition={{ duration: reduced ? 0 : 1.05, delay: reduced ? 0 : 0.08, ease: [0.22, 1, 0.36, 1] }}
          className="relative mx-auto min-h-[420px] w-full overflow-hidden border border-[#c99a3c]/20 bg-[#0a0907] shadow-[0_55px_140px_rgba(0,0,0,.72)] sm:min-h-[560px] lg:min-h-[650px]"
          aria-label="Prime Presence Atelier branded collateral"
        >
          <Image
            src="/media/ppa-hero-collateral.webp"
            alt="Prime Presence Atelier premium black and gold brand collateral staged on a dark marble surface"
            fill
            priority
            sizes="(max-width: 1024px) 100vw, 58vw"
            className="object-cover object-center"
          />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,3,2,.68)_0%,rgba(3,3,2,.16)_34%,transparent_64%),linear-gradient(0deg,rgba(0,0,0,.38),transparent_36%)]" />
          <div className="absolute bottom-5 left-5 border-l border-[#c99a3c]/60 pl-4 text-[9px] font-semibold uppercase tracking-[.28em] text-white/56 sm:bottom-8 sm:left-8">
            Identity · Presence · Launch
          </div>
        </motion.div>
      </div>
