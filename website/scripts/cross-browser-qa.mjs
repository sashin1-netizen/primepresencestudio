  ...
  try {
    for (const viewport of viewports) {
      ...
      for (const route of routes) {
        ...
        // Decorative geometry is intentionally allowed to bleed beyond its composition box.
        // Content overflow is checked here; media and controls have dedicated checks below.
        const overflowedVisibleElements=await page.locator('main *').evaluateAll(elements=>elements.filter(el=>{const style=getComputedStyle(el);if(style.display==='none'||style.visibility==='hidden'||style.position==='fixed')return false;const rect=el.getBoundingClientRect();if(rect.width===0||rect.height===0)return false;const outside=rect.left<-2||rect.right>window.innerWidth+2;if(!outside)return false;const text=el.textContent?.trim()||'';const isContentElement=/^(P|H1|H2|H3|H4|H5|H6|LI|BLOCKQUOTE|PRE|CODE|TABLE|FORM|LABEL)$/.test(el.tagName);return isContentElement&&text.length>0}).slice(0,5).map(el=>({tag:el.tagName,className:el.className,text:el.textContent?.trim().slice(0,80)})));if(overflowedVisibleElements.length)fail(browserName,viewport,route,`visible content extends beyond viewport: ${JSON.stringify(overflowedVisibleElements[0])}`);

        // scrollHeight/clientHeight alone is not evidence of clipping: font ink and line boxes
        // legitimately exceed the element box across engines. Only fail when CSS actually clips
        // an axis and the heading has hidden scrollable content on that same axis.
        const clippedHeadings=await page.locator('h1,h2,h3,h4').evaluateAll(elements=>elements.filter(el=>{const style=getComputedStyle(el);if(style.display==='none'||style.visibility==='hidden')return false;const clipsX=style.overflowX==='hidden'||style.overflowX==='clip';const clipsY=style.overflowY==='hidden'||style.overflowY==='clip';return (clipsX&&el.scrollWidth>el.clientWidth+2)||(clipsY&&el.scrollHeight>el.clientHeight+2)}).slice(0,3).map(el=>el.textContent?.trim().slice(0,100)));if(clippedHeadings.length)fail(browserName,viewport,route,`heading clipping detected: ${clippedHeadings[0]}`);
